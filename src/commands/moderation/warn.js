...
module.exports = {
  data: new SlashCommandBuilder()
    ...
    .addSubcommand((s) =>
      s
        ...
        .addIntegerOption((o) =>
          o
            .setName("duration")
            .setDescription("Duration for the warning to last (in minutes).")
        )
    )
    .addSubcommand((s) =>
      s
        .setName("check")
        .setDescription("Check a warning to from user.")
        .addUserOption((o) =>
    ...
    .addSubcommand((s) =>
      s
        .setName("delete")
        .setDescription("Delete a warning to from user.")
        .addUserOption((o) =>
          o
            .setName("user")
            .setDescription("The user to warn.")
        ...
        .addIntegerOption((o) =>
          o
            .setName("index")
            .setDescription(
              "The index of the warning to delete (use /warn check to find the index)."
            )
        )
    )
    .toJSON(),
    ...
    try {
      ...
      if (!moderationDocument)
        return interaction.reply({
          content:
            "`⚠️` Please set up the moderation system for this guild before using the warn command.",
          ephemeral: true,
        });
      /// OPTIONAL
      ...
      if (subcommand === "add") {
          ...
          const warningPages = await Promise.all(
            warnings.map(async (warning, index) => {
              ...
              const embed = new EmbedBuilder()
                .setColor(mConfig.embedColorSuccess)
                .setTitle(
                  `\`⚠️\` ${user.tag}'s Warnings - Warning ${warningIndex} `
                )
                .addFields(
                  {
                    name: "Moderator",
        ...
        try {
          ...
          if (index >= 0 && index < warnings.length) {
            ...
            return interaction.reply({
              content:
                "`❌` Invalid warning index. Please provide a valid index.",
              ephemeral: true,
            });
          }
        } catch (error) {
          console.error(error);
          return interaction.reply({
            content: "`❌` an error occured while processing the command.",
            ephemeral: true,
          });
        }
      } else {
